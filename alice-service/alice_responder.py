"""Alice responder - generates answers to meeting questions using Claude."""
...
class AliceResponder:
    ...
    def _load_prompt_template(self) -> None:
        """Load the Alice system prompt template from file."""
        current_dir = os.path.dirname(os.path.abspath(__file__))
        prompt_path = os.path.join(current_dir, 'alice-prompt.md')

        try:
            with open(prompt_path, 'r') as f:
                self.system_prompt = f.read()
            logger.info("Loaded Alice system prompt")
        except Exception as e:
            logger.error(f"Failed to load Alice prompt template: {e}")
            # Fallback to basic prompt
            self.system_prompt = "You are Alice, a helpful meeting assistant. Answer questions about the meeting concisely."
    ...
    def answer(
        ...
        Returns:
            ...
            - answer: The text answer from Alice
